...
export default function BraceletsPage() {
  ...
  const handleCreate = async (event) => {
    ...
    setFeedback('');
    try {
      await api.post('/bracelets', { number });
      setNumber('');
      await loadBracelets();
  ...
  const openComanda = async (braceletNumber) => {
    setFeedback('');
    try {
      await api.post('/comandas/open', { number: braceletNumber });
      setFeedback('Comanda aberta com sucesso');
      await loadBracelets();
    } catch (err) {
      setFeedback(err.response?.data?.message || 'Erro ao abrir comanda');
    }
  };
                ...
                <div key={bracelet.id} className="rounded-[1.75rem] border border-slate-800/90 bg-slate-950/95 p-5">
                  ...
                  {comanda ? (
                    ...
                    <div className="mt-5 space-y-4">
                      ...
                      <button
                        type="button"
                        onClick={() => openComanda(bracelet.number)}
